...
//	EngFuncs::KEY_ClearStates ();
	if( !uiStatic.client.IsActive() )
	{
		...
		// diffusion
		if( EngFuncs::GetCvarFloat( "ui_is_active" ) > 0 )
			EngFuncs::CvarSetValue( "ui_is_active", 0 );
	}
}
...
{
	if( !EngFuncs::FileExists( "data/chapterbackgrounds.txt", TRUE ))
		return;

	char *afile = (char *)EngFuncs::COM_LoadFile( "data/chapterbackgrounds.txt", NULL );
	char *pfile = afile;
	char token[1024];
...
int UI_VidInit( void )
{
	...
	{
		uiStatic.scaleX = uiStatic.scaleY = ScreenWidth / 1920.0f;// 1024.0f;
		uiStatic.yOffset = ( ScreenHeight / 2.0f ) / uiStatic.scaleX - 1080.0f / 2.0f;  // 768.0f
	}
	else
	{
		// Sizes are based on screen height
		uiStatic.scaleX = uiStatic.scaleY = ScreenHeight / 1080.0f;  // 768
		uiStatic.yOffset = 0;
	}
