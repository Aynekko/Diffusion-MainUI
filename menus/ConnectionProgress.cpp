...
public:
	...
	void HandleConnect( const char *pszText )
	{
		if( !pszText )
		{
			...
			SetServer( "" );
			SetCommonText( L( "Starting server..." ) );
		}
		else
		{
			m_iSource = SOURCE_SERVERBROWSER;
			SetServer( pszText );
			SetCommonText( L( "Establishing connection..." ) );
		}
	...
	void SetServer( const char *pszName )
	{
		...
		{
			Q_strncpy( sTitleString, L( "Starting server..." ), sizeof( sTitleString ) );
		}
		else if( !pszName[0] )
		{
			Q_strncpy( sTitleString, L( "Starting server..." ), sizeof( sTitleString ) );
		...
		else
		{
			snprintf( sTitleString, sizeof( sTitleString ) - 1,
				"%s %s", L( "Establishing connection..." ), pszName );
		}
